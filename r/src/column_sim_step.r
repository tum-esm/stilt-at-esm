...
#' VERSION 2.0 - fixed the out_the_box issue -tsj
#' @author Ben Fasoli
#'
...
column_sim_step <- function(before_footprint = list(function() {output}),
  ...
  try({
    ...
    r_designator <- unlist(r_designator)

    # Validate arguments
    if (!run_trajec && !run_foot)
      stop('column_sim_step(): Nothing to do, set run_trajec or run_foot to T')
    ...
    #File format for em27 version:
    if (is.na(simulation_id)) {
				simulation_id_format <- paste0(r_designator,'_%Y%m%d%H%M_',r_zagl,'m')
    		simulation_id <- strftime(r_run_time,format = simulation_id_format,tz = 'UTC')
    }
    ...
    if (run_trajec) {
      ...
      if (!file.exists(output$file)) {
        warning('column_sim_step(): No _traj.rds file found in ', rundir,
    ...
    out_the_box <- subset(output$particle, (lati<=ymn) | (lati>=ymx) | (long<=xmn) | (long>=xmx),c(indx,time) )
    exit_times <- r_run_time + 60*aggregate(out_the_box,by=list(out_the_box$indx),FUN=max)$time
    bbox_file <- file.path(rundir, paste0(basename(rundir), '_bbox.rds'))

    saveRDS(exit_times,bbox_file)
